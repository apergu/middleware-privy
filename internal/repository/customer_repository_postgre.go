...
func (c *CustomerRepositoryPostgre) FindByCustId(ctx context.Context, customerId string, tx pgx.Tx) (entity.Customer, error) {
	var cmd sqlcommand.Command = c.pool
	if tx != nil {
		cmd = tx
	}
	...
	return c.queryOne(ctx, cmd, query, customerId)
}
...
func (c *CustomerRepositoryPostgre) FindByEmail(ctx context.Context, email string, tx pgx.Tx) (entity.Customer, error) {
	var cmd sqlcommand.Command = c.pool
	if tx != nil {
		cmd = tx
	}
	...
	return c.queryOne(ctx, cmd, query, email)
}
...
func (c *CustomerRepositoryPostgre) UpdateLead2(ctx context.Context, id int64, cust entity.Customer, tx pgx.Tx) error {
	var cmd sqlcommand.Command = c.pool
	if tx != nil {
		cmd = tx
	}
...
// 	log.Println("ID : ", id)
// 	log.Println("cust : ", cust.EnterprisePrivyID)
