...
func (c *CredentialPrivy) CreateApplication(ctx context.Context, param ApplicationParam) (ApplicationResponse, error) {
	// get jwt
	isNode := true
	jwtToken := JWTToken{}
	var err error
	...
	if err != nil {
		logrus.
			WithFields(logrus.Fields{
				"at":  "CredentialPrivy.CreateApplication",
				"src": "JWTToken{}",
			}).
			Error(err)
	...
	logrus.
		WithFields(logrus.Fields{
			"at":                    "CredentialPrivy.CreateApplication",
			"src":                   "EnvelopeApplication{}.beforeDo",
	...
	// post customer
	postApplicationURL := c.host + EndpointPostCustomer
	...
	logrus.
		WithFields(logrus.Fields{
			"at":   "CredentialPrivy.CreateApplication",
			"src":  "EnvelopeApplication{}.beforeDo",
			"host": postApplicationURL,
		}).
		Info(body.String())

	req, _ = http.NewRequest(http.MethodPost, postApplicationURL, body)
