...
func (c *CredentialPrivy) GetDataRelateCustomerUsage(ctx context.Context, param GetCustUsageParam) (string, error) {
	isNode := true
	jwtToken := JWTToken{}
	var err error
	...
	params := url.Values{}
	params.Add("customer_id", param.CustomerID)
	// params.Add("param2", "value2")

	// post customer
	postCustomerURL := c.host + EndpointPostCustomer + "?" + params.Encode()

	req, _ = http.NewRequest(http.MethodGet, postCustomerURL, nil)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", credential.TokenType+"?"+credential.AccessToken)
